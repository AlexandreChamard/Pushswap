...
** Last update Sun Nov 27 11:20:12 2016 Alexandre Chamard-bois
*/
...
int verif(t_array array)
{
  ...
  {
    if (i != 0)
      if (array.nb[i][1] < array.nb[i - 1][3] && array.nb[i][0] >= 1)
        return (1);
...
int **create_array(t_array *array, char **av)
{
  ...
  if ((new_array = malloc(sizeof(int *) * (array->nb_array))) == NULL)
    return (NULL);
  i = 0;
  while (i < array->nb_array)
  {
    if ((new_array[i] = malloc(sizeof(int) *
        (C_SIZE(array->nb_array - i - 1, array->size_last) + 1))) == NULL)
      return (NULL);
...
int main(int ac, char **av)
{
  ...
  if (verif(array) != 0)
  {
    ...
    {
      if ((array.nb = lets_merge(&array)) == NULL)
      return (84);
